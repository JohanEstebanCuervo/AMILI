...
class CreateWidgets:
    ...
    def create_label(self, text: str, row: int, column: int = 0) -> ttk.Label:
        """
        ...
        label.grid(row=row, column=column, sticky="nes", padx=2, pady=2)

        return label

    def create_widget(self, value, variable, row: int, column: int = 1) -> object:
        """
        ...
        widget.grid(row=row, column=column, sticky="nesw", padx=2, pady=2)
...
class Table(ttk.Frame, CreateWidgets):
    """
    ...
    def __init__(
        ...
        super(ttk.Frame).__init__(master)
        super(CreateWidgets).__init__()
    ...
    def __create_widgets(self, table: pd.DataFrame) -> None:
        ...
        for index_row, row_data in enumerate(table.iterrows()):
            ...
            for index_col, value in enumerate(row_data[1]):
                ...
                widget = self.create_widget(value, variable, index_row, index_col)
...
class Form(ttk.LabelFrame, CreateWidgets):
    ...
    def __init__(
        ...
        super(ttk.LabelFrame, self).__init__(master, widgetname=".!labelframe")
